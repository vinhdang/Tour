    ...
    public class AreaController : Controller
    {
        ...
        public ActionResult Edit(string id)
        {
            ...
            if (info != null)
            {
                ...
                area.Districts = districtService.All.Where(p => p.ProvinceID == info.ProvinceID).ToSelectListItems(info.DistrictID);
                return View(area);
            }
        ...
        {
            int areaID = Protector.Int(id);
            Area info = areaService.Get(d => d.AreaID == areaID);
            if (info != null)
            {
                if (!ModelState.IsValid)
                {
                    ...
                    current.Districts = districtService.All.Where(p => p.ProvinceID == info.ProvinceID).ToSelectListItems(info.DistrictID);
                    return View(current);
                }
                TryUpdateModel(info);
                areaService.Save();
